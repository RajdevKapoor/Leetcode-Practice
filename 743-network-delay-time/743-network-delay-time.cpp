...
public:
    int networkDelayTime(vector<vector<int>>& times, int n, int k) {
        ...
        while(pq.size()){
            int distanceUptoParent = pq.top().first;
            ...
            for(auto nbr:graph[parent]){
                ...
                if(distanceUptoParent + distanceParentToChild < distTo[childNode]){
                    distTo[childNode]=distanceUptoParent + distanceParentToChild;
